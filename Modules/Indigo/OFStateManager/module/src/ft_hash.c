...
ft_hash_create(ft_config_t *config)
{
    ...
    ft->match_map = hmap_create(match_hash, match_equality,
                                offsetof(struct ft_entry_s, match), 0);

    ft->flow_id_map = hmap_create(hmap_uint64_hash, hmap_uint64_equality,
                                  offsetof(struct ft_entry_s, id), 0);

    ft->priority_map = hmap_create(hmap_uint16_hash, hmap_uint16_equality,
...
ft_hash_delete(ft_instance_t ft)
{
    ...
    if (ft->flow_id_map != NULL) {
        hmap_destroy(ft->flow_id_map);
    }

    if (ft->priority_map != NULL) {
        hmap_destroy(ft->priority_map);
    }

    if (ft->match_map != NULL) {
        hmap_destroy(ft->match_map);
    }
